...
int USBRL_API usb_just_test(void);
...
//==========================================================================================================
/* Additional functions for using with .NET platform - without explicit handles managing
*/

/** Initializing library - this function should be called before all other functions
*  Creates Linked list of device handlers, count total device count and so on
@return 0 if was successfull, otherwise 1
*/
int USBRL_API rl_init(void);

/** Finalizing library work - clear Linked List */
void USBRL_API rl_finalize(void);

/* Returns number of found devices. rl_init() should be called before! 
*  Please check docs - this library works with only one kind of relays.
@return 0 if no device found, or total number of found devices
*/
int  USBRL_API rl_count(void);

/*  Close active device if it is opened
@return 1 if failed for some reason, 0 on success
*/
int  USBRL_API rl_closeDevice(void);

/* Selects device from List by index. If current device is opened, it will be closed
@param newDeviceIndex - index of device to select, starting from 0
@return 0 on success, 1 if not inited, 2 on wrong newDeviceIndex value
*/
int USBRL_API rl_rewind(int newDeviceIndex);

/* Opens current device and count it's channels
@return 0 on success, 1 if not inited, 2 if there is no device detected, 3 if failed to open
*/
int USBRL_API rl_openDevice(void);

/* Just returns value of 'channelCount' global variable
@return Number of channels on active device. -1 if not inited or no device detected, -1 if not opened
*/
int USBRL_API rl_channelsCount(void);

/** Turn ON a relay channel on the USB-Relay-Device
@param channelIndex -- the channel number: 1...max
@returns: 0 - success; 1 - error; 2 - wrong index
*/
int USBRL_API rl_openChannel(int channelIndex);

/** Turn OFF a relay channel on the USB-Relay-Device
@param channelIndex -- which channel your want to close
@returns: 0 - success; 1 - error; 2 - wrong index
*/
int USBRL_API rl_closeChannel(int channelIndex);

/** Turn ON all relay channels on the USB-Relay-Device
@returns: 0 - success; 1 - error
*/
int USBRL_API rl_openAllChannels(void);

/** Turn OFF all relay channels on the USB-Relay-Device
@returns: 0 - success; 1 - error
*/
int USBRL_API rl_closeAllChannels(void);

/* Get the ID string of the device (typically 5 characters)
@param charBuffer -- pointer to C-string buffer
@param buffSize -- hold maximum buffer size
@returns C-string in charBuffer with 0-terminator, if buffer size too small then serial will be truncated to fit
*/
void USBRL_API rl_getSerialNumber(char * charBuffer, size_t buffMaxSize);

/* Get status of all relays on the device.
* @return Bitmask of all relay channels state, if the value > 0. Negative values mean error.
*  bit 0/1/2/3/4/5/6/7/8 indicate channel 1/2/3/4/5/6/7/8 status
*  Each bit value 1 means ON, 0 means OFF.
* @note Relay should be opened, otherwise returns -1.
*/
int USBRL_API rl_status(void);

/* Indicates init status, 1 - relay was inited, 0 - failed to init*/
int USBRL_API rl_inited(void);

/* Returns current device index, from 0 to relay count - 1. If something wrong - returns -1 */
int USBRL_API rl_current_device_index(void);
