...
int USBRL_API usb_just_test(void) { return 2018; }

///  Pointer to array of device structures
pusb_relay_device_info_t pt;

///  Indicator of successfull initialization
int successInit = 0;
...
/// Current device
pusb_relay_device_info_t currentDevice;

/// Current device index
int currentDeviceIndex;

/// Indicates if the current device was opened
int currentDeviceOpened;

/// Total channels of active device
int channelCount;

void rl_finalize() {
    if (pt != 0) usb_relay_device_free_enumerate(pt);
	pt = 0;
    usb_relay_exit();
}

int rl_count() { return deviceCount; }

/// Initializing library - this function should be called before all other functions
int rl_init() {

	/// Initialize library and check status
    pt = 0;
    deviceCount = 0;
    currentDeviceIndex = 0;
    currentDevice = 0;
    currentDeviceOpened = 0;
    channelCount = 0;
    successInit = 0;

	///  Performing library init
	int errorCode = usb_relay_init();
    if (errorCode != 0) return errorCode;

    successInit = 1;

    // Getting array of device structures
    pt = usb_relay_device_enumerate();

    // Counting relays
    pusb_relay_device_info_t q = pt;
    while (q) {
        ++deviceCount;
        q = q->next;
    }
...
//  Close active device
int rl_closeDevice() {
    if (successInit == 0) return 1;
    if (currentDeviceOpened) usb_relay_device_close((intptr_t)(currentDevice));
    currentDeviceOpened = 0;
    return 0;
}

//  Rewind to starting device in list
int rl_rewind(int newDeviceIndex) {
    if (successInit == 0) return 1;
    if (newDeviceIndex < 0 || newDeviceIndex >= deviceCount || newDeviceIndex == currentDeviceIndex) return 2;

    rl_closeDevice();

    currentDeviceIndex = newDeviceIndex;
    currentDevice = pt;
    while (newDeviceIndex > 0) {
        --newDeviceIndex;
...
/// Opens current device and count it's channels
int rl_openDevice() {
    if (successInit == 0) return 1;
    if (deviceCount <= 0) return 2;
	if (usb_relay_device_open(currentDevice) == NULL) return 3;
    currentDeviceOpened = 1;

    channelCount = (unsigned char)currentDevice->type;
...
int rl_channelsCount() {
	if (successInit == 0 || deviceCount == 0) return -1;
	if (currentDeviceOpened == 0) return -2;
	return channelCount;
}

int rl_openChannel(int channelIndex) {
    if (successInit == 0 || deviceCount == 0 || currentDeviceOpened == 0) return 1;
    if (channelIndex <= 0 || channelIndex > channelCount) return 2;
    return usb_relay_device_open_one_relay_channel((intptr_t)currentDevice, channelIndex);
}

int rl_closeChannel(int channelIndex) {
	if (successInit == 0 || deviceCount == 0 || currentDeviceOpened == 0) return 1;
	if (channelIndex <= 0 || channelIndex > channelCount) return 2;
	return usb_relay_device_close_one_relay_channel((intptr_t)currentDevice, channelIndex);
}

int rl_openAllChannels() {
	if (successInit == 0 || deviceCount == 0 || currentDeviceOpened == 0) return 1;
    return usb_relay_device_open_all_relay_channel((intptr_t)currentDevice);
}

int rl_closeAllChannels() {
	if (successInit == 0 || deviceCount == 0 || currentDeviceOpened == 0) return 1;
    return usb_relay_device_close_all_relay_channel((intptr_t)currentDevice);
}

void rl_getSerialNumber(char * charBuffer, size_t buffMaxSize) {
    if (successInit == 0 || buffMaxSize < 2) {
        return;
    }
    size_t serial_length = strlen(currentDevice->serial_number);
	if (serial_length > USB_RELAY_ID_STR_LEN)
		serial_length = USB_RELAY_ID_STR_LEN;
    if (serial_length + 1 > buffMaxSize)
        serial_length = buffMaxSize - 1;

    strncpy(charBuffer, currentDevice->serial_number, serial_length);
	charBuffer[serial_length] = 0;
}

int rl_status() {
    if (successInit == 0 || deviceCount == 0) return -1;
    if (currentDeviceOpened == 0) return -1;
    unsigned st = 0;
    if(usb_relay_device_get_status((intptr_t)currentDevice, &st)==0)
		return st;
	return -1;
};
